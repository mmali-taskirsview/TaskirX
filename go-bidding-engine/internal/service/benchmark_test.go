...
func BenchmarkCreativeOptimization_SelectCreative(b *testing.B) {
	...
	for i := 0; i < 100; i++ {
		...
		if i%10 == 0 {
			svc.RecordClick("creative-"+string(rune(i%3)), "")
		}
	}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		svc.SelectCreative(campaign, req)
	}
}
...
func BenchmarkBidLandscape_AnalyzeLandscape(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		svc.AnalyzeLandscape(campaign, req)
	}
}
...
func BenchmarkDayparting_CalculateDaypartMultiplier(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		svc.CalculateDaypartMultiplier(campaign, req)
	}
}
...
func BenchmarkBidCache_Get(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		svc.Get(context.Background(), "test-key")
	}
}
...
func BenchmarkDirectPublisher_GetPublisher(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		svc.GetPublisher(registered.ID)
	}
}
