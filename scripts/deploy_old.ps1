...
#    Write-Host "Switched Terraform path to ./terraform-oci for OCI environment" -ForegroundColor Blue
# }

Write-Host "DEBUG: Line 40 reached - Post OCI check" -ForegroundColor Magenta

Write-Host "DEBUG: Reaching Test-Prerequisites area..." -ForegroundColor Cyan
...
Write-Host "Function Test-Prerequisites defined." -ForegroundColor Cyan
...
function Main {
    ...
    # Check if Write-Success is available
    if (Get-Command "Write-Success" -ErrorAction SilentlyContinue) {
        Write-Host "Write-Success is available." -ForegroundColor Green
    } else {
        Write-Host "Write-Success is NOT available." -ForegroundColor Red
    }
